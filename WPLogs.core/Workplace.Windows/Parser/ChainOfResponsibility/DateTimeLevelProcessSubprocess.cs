...
{
    public partial class DateTimeLevelProcessSubprocess : SingleLineParser
    {
        const string pattern = @"^(?<year>\d{4})-(?<month>\d{2})-(?<day>\d{2})\s(?<hour>\d{2}):(?<minute>\d{2}):(?<second>\d{2}).(?<milisecond>\d{3})\s(?<level>\w{1})\s\[(?<process>.+?)\]\s\[(?<subprocess>.+?)\]\s(?<body>.+)";
